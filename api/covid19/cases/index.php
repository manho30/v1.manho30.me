...
if ($date_request == "")
  {
    $json = array(
        "Status" => "Fail",
        "Message" => "Did not provide query string - date. Please refer to https://covid-19.samsam123.name.my/api.html for more information."
    );
                ...
                if ($array_number == "")
                  {
                    $json = array(
                        "Status" => "Fail",
                        "Message" => "Could not find state provided! Please try again!"
                    );
                    header('Content-type: application/json');
                    header('Access-Control-Allow-Origin: *');
                    echo (json_encode($json));
            ...
            if ($output_date_data !== $date_request)
              {
                $json = array(
                    "Status" => "Fail",
                    "Message" => "Could not find date provided! Please try again! Please try after midnight if you wanted to retrieve the latest data!"
                );
                header('Content-type: application/json');
                header('Access-Control-Allow-Origin: *');
                echo (json_encode($json));
